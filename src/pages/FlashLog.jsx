    ...
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "2rem 1.5rem" }}>
      ...
      <div style={{ marginBottom: "2rem" }}>
        ...
        <p style={{ color: "#3a5a7a", fontSize: "0.8rem", margin: "4px 0 0" }}>
          {getIST()} IST &middot; {flashLogs.length} vehicles flashed &middot; {availableVehicles.length} remaining
        </p>
      </div>
                ...
                {filteredVehicles.length === 0
                  ...
                  : filteredVehicles.map((v) => (
                    ...
                    >
                      ...
                      <span style={{ color: "#3a5a7a", marginLeft: 8 }}>{v.zone} &middot; {v.customer_name}</span>
                    </div>
                  ))
                }
              </div>
                ...
                {last10.map((log) => (
                  <div key={log.id} style={{ padding: "8px 12px", background: "#0a1525", borderRadius: 6, border: "1px solid #1e2d4a", fontSize: "0.78rem" }}>
                    ...
                    <div style={{ color: "#5a7a9a", marginTop: 2 }}>
                      {log.zone} &middot; {log.customer_name} &middot; <span style={{ color: "#7a9aba" }}>{log.old_version} to {log.new_version}</span> &middot; {log.technician_name}
                    </div>
                  </div>
                ))}
              </div>
